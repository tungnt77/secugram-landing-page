...
const LogoMark = ({ color = '#1e3a5f' }) => (
  <svg width="38" height="38" viewBox="0 0 100 100" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    <circle cx="50" cy="43" r="9" fill="none" stroke="#0d9488" strokeWidth="3.5"/>
    <path d="M43 50 L43 61 L57 61 L57 50 Z" fill="none" stroke="#0d9488" strokeWidth="3.5" strokeLinejoin="round"/>
  </svg>
);
...
const NavBar = ({ activeSection, onNav }) => {
  ...
  return (
    <nav data-tw-nav
      ...
      <div style={{ display: 'flex', alignItems: 'center', cursor: 'pointer' }} onClick={() => onNav('hero')}>
        <img
          src="long_logo.png"
          alt="Secugram"
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 24 }}>
        {navItems.map(item => (
          <a key={item.target} data-tw-nav-link data-tw-nav-cta={item.isCta ? 'true' : undefined} onClick={() => onNav(item.target)}
            style={{
              fontFamily: "'Montserrat',sans-serif", fontWeight: 600, fontSize: 13,
              color: item.isCta ? '#fff' : (activeSection === item.target ? '#1e3a5f' : '#6b7280'),
              background: item.isCta ? '#0d9488' : 'transparent',
              textDecoration: 'none', cursor: 'pointer',
              borderBottom: item.isCta ? 'none' : (activeSection === item.target ? '2px solid #0d9488' : '2px solid transparent'),
...
const HeroGraphic = () => (
  <svg width="320" height="300" viewBox="0 0 320 300" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    <circle cx="160" cy="155" r="3" fill="#0d9488"/>
  </svg>
);
...
const HeroSection = ({ onNav }) => {
  ...
  <div data-tw-hero-grid style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', minHeight: 560 }}>
    ...
    <div data-tw-hero-left style={{
      ...
      <div data-tw-accent-text style={{ fontFamily: "'Montserrat',sans-serif", fontWeight: 700, fontSize: 12, color: '#0d9488', letterSpacing: '0.14em', textTransform: 'uppercase', marginBottom: 20 }}>
      ...
      <h1 data-tw-hero-h1 data-tw-h1 style={{ fontFamily: "'Montserrat',sans-serif", fontWeight: 800, fontSize: 48, color: '#1e3a5f', lineHeight: 1.08, letterSpacing: '-0.02em', margin: '0 0 22px' }}>
        Enterprise-Grade Cybersecurity<br />for <span data-tw-accent-text style={{ color: '#0d9488' }}>Growing Businesses</span>
      </h1>
      ...
      <div data-tw-hero-ctas style={{ display: 'flex', gap: 14, alignItems: 'center' }}>
        ...
        <a onClick={() => onNav('services')} style={{
          ...
          borderBottom: '1.5px solid #1e3a5f', paddingBottom: 1,
        }}>Explore Services →</a>
      </div>
    ...
    <div data-tw-hero-right style={{
      ...
      <div style={{ position: 'absolute', width: 680, height: 520, background: 'radial-gradient(circle, rgba(13,148,136,0.28) 0%, rgba(13,148,136,0.08) 38%, transparent 72%)', borderRadius: '50%', transform: 'translateX(-20px)' }}/>
