...
const FooterLogoMark = () => (
  <svg width="32" height="32" viewBox="0 0 100 100" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    <path d="M44 51 L43 63 L57 63 L56 51" fill="none" stroke="#0d9488" strokeWidth="3" strokeLinejoin="round"/>
  </svg>
);

const FooterSection = ({ onNav }) => (
  <footer style={{ background: '#0d1b2e', padding: '64px 80px 32px', color: '#fff' }}>
    <div style={{ maxWidth: 1200, margin: '0 auto' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr', gap: 48, marginBottom: 56 }}>
        ...
        <div>
          ...
          {['Features', 'Pricing', 'Security', 'Changelog'].map(l => (
            <div key={l} style={{ fontFamily: "'Open Sans',sans-serif", fontSize: 14, color: '#94a3b8', marginBottom: 12, cursor: 'pointer' }}>{l}</div>
          ))}
        </div>
        {/* Company col */}
        <div>
      ...
      <div style={{ borderTop: '1px solid rgba(255,255,255,0.08)', paddingTop: 24, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div style={{ fontFamily: "'Open Sans',sans-serif", fontSize: 13, color: '#4b5563' }}>© 2025 Secugram JSC. All rights reserved. · 272 Nguyen Khoai, Hai Ba Trung, Hanoi</div>
        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          <div style={{ width: 8, height: 8, borderRadius: '50%', background: '#0d9488' }}></div>
          <span style={{ fontFamily: "'Open Sans',sans-serif", fontSize: 13, color: '#4b5563' }}>All systems operational</span>
        </div>
      </div>
    </div>
  </footer>
